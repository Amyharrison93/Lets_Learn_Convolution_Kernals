...
def identity_kernal(image: np.ndarray, kernel_size: int=3) -> np.ndarray:
    """
    Apply an identity kernel to the input image. The identity kernel is a 3x3 matrix that leaves the image unchanged when applied.
    warning: this is very slow and just an example of how to implement a convolution kernel from scratch. For practical use, consider using the identity_kernal_cv2 function which utilizes OpenCV's optimized filter2D function.
    Parameters:
    image (numpy.ndarray): The input image to which the identity kernel will be applied.
    kernel_size (int): The size of the identity kernel (default is 3).
    Returns:
...
if __name__ == "__main__":
    ...
    output_image = identity_kernal(input_image)
    end_time = time.time()
    print(f"Output image shape: {output_image.shape}")
    print(f"Time taken (custom implementation): {end_time - start_time:.4f} seconds")

    start_time = time.time()
    output_image_cv2 = identity_kernal_cv2(input_image)
    end_time = time.time()
